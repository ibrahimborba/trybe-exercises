...
const messageBeer = () => 'Beba Trybeer, suave como um log de erro vazio';
const messageChoco = () => 'ChocoTrybe, melhor que console.log';

const sendMessage = (array) => {
  return array.every((element) => element >= 18) ? messageBeer() : messageChoco();
}

console.log(sendMessage(arrayIdades));
console.log(sendMessage(arrayIdades2));
...
// escreva sua função aqui.
const messageNotAproved = () => 'Bora pra mentoria de projetos!';
const messageAproved = () => 'Pode abrir o suco de laranja que temos 100% de aprovação!';

const checkStatus = (array, grade) => {
  return array.every(element => element.status > grade) ? messageAproved() : messageNotAproved();
}

console.log(checkStatus(estudantes, 80));
